...
Profile.userinfo = async (userID) => {
  try {
    const result = await db.query(
      "SELECT *, REPLACE(users.image, 'https://storage.googleapis.com/wiseassist-b8a8a.appspot.com/profiles/', '') AS image, roles.role FROM users INNER JOIN roles ON users.role_id = roles.id WHERE users.id = $1 and users.is_deleted = false;",
      [userID]
    );
...
Profile.addlesson = async (userID, lessonID) => {
  try {
    ...
    const registerQuery =
      "INSERT INTO watched_videos (lesson_id, user_id) VALUES ($1, $2) RETURNING *";
...
Profile.addwish = async (userID, courseID) => {
  try {
    ...
    if (isFoundResult.rows.length === 0) {
      throw new Error(
        "The specified course has been deleted or does not exist."
      );
    }

    const insertWishlistQuery =
      "INSERT INTO witchlist (course_id, user_id) VALUES ($1, $2)";
